...
export const commonPrompts = {
  code_review: {
    ...
    async generate(args: { code: string; language?: string }): Promise<string> {
...
Please provide specific, actionable feedback with examples where appropriate.`;
  ...
  documentation: {
    ...
    async generate(args: { code: string; format?: string }): Promise<string> {
...
${args.format === 'jsdoc' ? '- JSDoc format comments' : ''}`;
  ...
  refactor: {
    ...
    async generate(args: { code: string; goals?: string }): Promise<string> {
