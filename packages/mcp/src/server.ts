...
class CustomMCPServer {
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ListResourcesRequestSchema, async () => {
      return {
        resources: Array.from(this.resources.entries()).map(([name, resource]: [string, any]) => ({
          uri: resource.uri,
          name: resource.name,
    ...
    this.server.setRequestHandler(ReadResourceRequestSchema, async (request: ReadResourceRequest) => {
      ...
      const resource = Array.from(this.resources.values()).find((r: any) => r.uri === uri);
    ...
    this.server.setRequestHandler(ListPromptsRequestSchema, async () => {
      return {
        prompts: Array.from(this.prompts.entries()).map(([name, prompt]: [string, any]) => ({
          name,
